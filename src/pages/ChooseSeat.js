  ...
  const handleAvailableSeat = async () => {
    try {
      ...
      const requestSeat = await axios.post(
        `https://tikitz-v2.adaptable.app/yongki/movie/${slug}/seat`,
        {
  ...
  const handleCheckout = async () => {
    try {
      ...
      const requestBooking = await axios.post(
        `https://tikitz-v2.adaptable.app/yongki/ticket/seat`,
        {
          movieSlug: slug,
      ...
      if (requestBooking.data.data.paymentId) {
        const requestPayment = await axios.patch(
          `https://tikitz-v2.adaptable.app/yongki/ticket/purchase/${requestBooking.data.data.paymentId}`,
          {},
          {
            headers: {
              Authorization: `Bearer ${token}`,
